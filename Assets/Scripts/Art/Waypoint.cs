...
{
	public static bool clickedArt = false;
	private enum State
	{
		Idle,
		Focused,
	...
	public void Click()
	{
		...
		Debug.Log(_state + "------state val");
		
		_audio_source.Play();

		Debug.Log("Camera.main.transform.position--" + camParent.transform.position);

		camParent.transform.position 	= gameObject.transform.position;
		//clickedArt = true;

		//StartCoroutine(falseClickedArt());
	
	}

	IEnumerator falseClickedArt()
    {
